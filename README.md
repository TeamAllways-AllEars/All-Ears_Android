...
You also need to provide the base url for your SpringBoot backend server in `app/src/main/java/gdsc/allways/allears/presentation/subtitles/SubtitleService.kt` file. Make GCP VM for backend server, attach **static external IP** to the VM instance, and provide the info of static external IP to the `BASE_URL` val in `SubtitleService.kt` file.
